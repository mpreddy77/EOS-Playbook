...
void Marketplace::buy(account_name buyer, uint64_t productId)
{
    ...
    asset productPrice = asset(product.price, string_to_symbol(4, "EOS"));

    // first inline action
    action(permission_level{buyer, N(active)}, N(eosio.token), N(transfer), make_tuple(buyer, _self, productPrice, string(""))).send();  
    //action(vector<permission_level>(), N(eosio.token), N(transfer), make_tuple(buyer, _self, productPrice, string(""))).send();
    
    // second inline action
    action(permission_level{buyer, N(active)}, N(market),  N(additem), make_tuple(buyer, product.product_id, product.name)).send();
    //action(vector<permission_level>(), N(market), N(additem), make_tuple(buyer, product.product_id, product.name, product.power, product.health, product.ability, product.level_up)).send();
